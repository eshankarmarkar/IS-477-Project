...
CLEAN_PATH = Path("data/clean/merged_affordability.csv")
...
def main():
    ...
    df_year = df.groupby("year", as_index=False)[num_cols].mean().sort_values("year").dropna(subset=["median_income_real","avg_sales_price_real","afford_ratio_real"])
    ...
    base_income = base["median_income_real"].iloc[0]
    base_price = base["avg_sales_price_real"].iloc[0]
    income_idx = df_recent["median_income_real"] / base_income * 100
    price_idx = df_recent["avg_sales_price_real"] / base_price * 100
    plt.figure(figsize=(8, 4.5))
    ...
    plt.close()
    dec = df_year.assign(decade=(df_year["year"] // 10) * 10).groupby("decade", as_index=False)["afford_ratio_real"].mean()
    plt.figure(figsize=(8, 4.5))
